...
func main() {
	...
	// CLI command to print the ephd version
	rootCmd.AddCommand(version.Cmd())
	// CLI command to start and run the ephd (server)
...
func exportAppStateAndTMValidators(
	...
	if height != -1 {
		gApp := app.NewEphApp(logger, db, traceStore, false, uint(1))
	...
	gApp := app.NewEphApp(logger, db, traceStore, true, uint(1))
	return gApp.ExportAppStateAndValidators(forZeroHeight, jailWhiteList)
}
